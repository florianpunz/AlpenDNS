...
use alpendns::filter::{Filter, FilterBackend, FilterSet, Lists, Verdict};
...
async fn the_second_load_gets_a_304_and_uses_the_cached_copy() {
    ...
    assert_eq!(second.block.len(), 2, "die Liste ging beim 304 verloren");
    assert_eq!(
...
async fn a_later_failure_keeps_the_server_running() {
    ...
    assert_eq!(set.block.len(), 0, "es gab nichts zu laden");
}
...
async fn filter_with(block: &str, allow: &str, mode: BlockMode) -> Arc<Filter> {
    ...
    let lists = Lists::new(
        Loader::new(dir.path()).expect("Loader"),
        vec![spec("block", Source::File(block_path), Format::Wildcard)],
        vec![spec("allow", Source::File(allow_path), Format::Wildcard)],
    );
    let set = lists.load(true).await.expect("Listen laden");
    Arc::new(Filter::new(set, &blocking_config(mode)))
}
...
async fn a_blocked_name_never_reaches_the_upstream() {
    ...
    let backend = FilterBackend::new(filter, upstream);

    let response = backend
        .resolve(&ask("ads.example.com."))
        .await
        .expect("Antwort");
...
async fn an_unlisted_name_goes_through() {
    let filter = filter_with("ads.example.com\n", "", BlockMode::Nxdomain).await;
    let backend = FilterBackend::new(filter, Upstream::default());

    let response = backend
        .resolve(&ask("example.org."))
        .await
        .expect("Antwort");
...
async fn the_allowlist_beats_the_blocklist() {
    ...
    .await;
    let backend = FilterBackend::new(Arc::clone(&filter), Upstream::default());
...
async fn the_verdict_names_the_rule_that_matched() {
    let filter = filter_with("# Kopf\nads.example.com\n", "", BlockMode::Nxdomain).await;
    let name = Name::from_ascii("sub.ads.example.com.").expect("gültig");

    let Verdict::Blocked(hit) = filter.verdict(&name) else {
        panic!("erwartet war ein Block");
    };
    assert_eq!(hit.matched, "ads.example.com");
    assert_eq!(filter.describe(&hit, false), "block:2");
}
...
async fn swapping_the_rules_under_load_neither_fails_nor_stalls() {
    ...
    let filter = filter_with("ads.example.com\n", "", BlockMode::Nxdomain).await;
    let backend = Arc::new(FilterBackend::new(Arc::clone(&filter), Upstream::default()));
    ...
    let swapper = {
        ...
        tokio::spawn(async move {
            ...
            while !stop.load(Ordering::SeqCst) {
                filter.replace(FilterSet::default());
